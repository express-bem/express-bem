...
function ExpressBem (opts) {
    ...
    // bundles path
    this.bundlesPath = PATH.resolve(this.projectRoot, this.relativePath);
 ...
 * @param {String[]} [targets] target extensions
 * @param {Function|Object} engine
 * @returns {ExpressBem}
 */
ExpressBem.prototype.engine = function (name, ext, targets, engine) {
    ...
    } else if (arguments.length === 3 && typeof targets === 'function') {
        engine = targets;
        targets = null;
    }
    ...
    if (targets) {
        engine.targetExtensions = targets;
    }
...
ExpressBem.prototype._initEnv = function (opts) {
    ...
    // ... and load enabled
    this.envLoadCache = globalCache || initVar('load', 'EXPRESS_BEM_CACHE_LOAD', env !== 'development');

    // ... and load enabled
    this.envExecCache = globalCache || initVar('exec', 'EXPRESS_BEM_CACHE_EXEC', env !== 'development');
